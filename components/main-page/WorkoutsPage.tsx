...
import { deleteWorkout, getWorkoutDates, getWorkoutsList, getWorkoutsOnDate } from '@/collections/Workouts/actions';
...
function WorkoutsDate({ date }: WorkoutsDateProps) {
      ...
      {getWorkoutsListQuery.data ? (
        <div className={'pb-12 space-y-6'}>
          {alwaysArray(getWorkoutsListQuery.data).map((date, index) => {
            return (
              <Collapsible key={date} defaultOpen={index === 0} asChild>
                <Card>
                  <CardHeader>
                    <CardTitle>
                      <CollapsibleTrigger className={'text-warning text-lg cursor-pointer'}>
                        {getReadableDate({ date }).readableDate}
                      </CollapsibleTrigger>
                    </CardTitle>
                  </CardHeader>
                  <CollapsibleContent>
                    <WorkoutsDate date={date} />
                  </CollapsibleContent>
                </Card>
              </Collapsible>
            );
